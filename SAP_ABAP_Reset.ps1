...
function Set-SAPPassword {
    ...
    try {
        ...
        #$cfgParams.Add($cfgParams::Trace, 10)
        ...
        [SAP.Middleware.Connector.IRfcFunction]$rfcFunction = $destination.Repository.CreateFunction("SUSR_USER_CHANGE_PASSWORD_RFC")
		
        Try {
            #-Metadata----------------------------------------------------
            $rfcFunction.SetValue("BNAME", $UserName)
            ...
            try {
                ...
                #Write-Output $rfcFunction
                #Write-Output "PreWorks"
                [SAP.Middleware.Connector.IRfcStructure]$export = $rfcFunction.GetValue("RETURN")
                $message = $export.GetValue("MESSAGE")
                $type = $export.GetValue("TYPE")
            }
            catch {
                #Write-Host $rfcFunction
                [SAP.Middleware.Connector.IRfcStructure]$export = $rfcFunction.GetValue("RETURN")
                Break
        ...
        if ($type -eq "S") {
            ...
            switch -wildcard ($resultsarray.ToString().ToLower()) {
                ...
                default { Write-Output "Failed to reset the local password for account '$UserName' on Host '$HostName'.Error = $message." }
            }
        }
    }
    catch {
        switch -wildcard ($error[0].Exception.ToString().ToLower()) {
            ...
            #Add other wildcard matches here as required
            default { Write-Output "Failed to reset the local Windows password for account '$UserName' on Host '$HostName'. Error = " $message }
        }
    }
}
